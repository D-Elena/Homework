...
function buyProduct(name = '') {
    if (name != '') {
        const item = shoppingList.findIndex((el) => el.nameProduct === name);
...
const newProduct = ['eggs', 'chicken', 'tea', 'coffee', 'water', 'duck'];
...
function addProduct(name, amount, price) {
    const item = secondShoppingList.findIndex((el) => el.nameProduct === name);
...
secondShoppingList.forEach(item => {
...
console.log(`Сумма всіх непридбаних продуктів ` + resultNotBought);
...
function listSort(list, decrease) { //уменьшение
    if (decrease == false) {
        list.sort((a, b) => {
